function [ Feats ] = fESSVEP( fch1, fch2, fch3, fch4, Fs, plotData )
...
if wL>=500
    ...
    F1=F(select);
%     [S1, F1]  = selectData(S,F,wlen,winLim);
%     [S, ~, ~] = stft2(fch(2,:),wlen,h,nfft,Fs,winLim);
%     [S, ~, ~] = stft2(fch(3,:),wlen,h,nfft,Fs,winLim);
%     [S, ~, ~] = stft2(fch(3,:),wlen,h,nfft,Fs,winLim);
    if plotData
%         subplot(3,2,3);hold on;imagesc(T,F1,S1),ylim(winLim),xlim([min(T),max(T)]);set(gca,'YDir','normal');colorbar;colormap(jet);
%         subplot(3,2,4);hold on;imagesc(T,F,S2),ylim(winLim),xlim([min(T),max(T)]);set(gca,'YDir','normal');colorbar;colormap(jet);
%         subplot(3,2,5);hold on;imagesc(T,F,S3),ylim(winLim),xlim([min(T),max(T)]);set(gca,'YDir','normal');colorbar;colormap(jet);
%         subplot(3,2,6);hold on;imagesc(T,F,S4),ylim(winLim),xlim([min(T),max(T)]);set(gca,'YDir','normal');colorbar;colormap(jet);
    end
end
%% TEMP
% Feats = zeros(8,1);
Feats = [S1(1,1),S1(2,1),S1(3,1),S1(4,1),F1(1,1),F1(1,2),F1(1,3),F1(1,4)];
